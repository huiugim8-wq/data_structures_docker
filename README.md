...
- 현재 열려 있는 로컬 프로젝트 폴더가 컨테이너 내부 `/workspaces/data_structures_docker` 경로로 **바인드 마운트**됩니다.
- 따라서 VSCode에서 파일을 수정하면 로컬 폴더와 컨테이너 내부 파일이 즉시 동일하게 반영됩니다.
- C 컴파일러(`gcc`, `g++`, `clang`)와 디버거(`gdb`, `valgrind`)는 컨테이너 이미지에 미리 설치됩니다.
...
4. 컨테이너 터미널에서 아래 명령어로 컴파일러가 잘 설치됐는지 확인할 수 있습니다.
